...
object KernelManager {
    ...
    fun initKernel(context: Context, authCode: String, needAutoSaveLicense: Boolean) {
        ...
        isFinished = false
        isSuccess = false
        installX5SelfHosted(context, authCode, needAutoSaveLicense)
    }

    /**
     * 安装或加载X5自运营版
     */
    private fun installX5SelfHosted(
        context: Context,
        authCode: String,
        ...
        if (needInstallOrUpdateX5) {
            val downloader = object : X5Downloader(context) {
                override fun onFinished() {
                    ...
                    doAuthAndInit(context, needAutoSaveLicense)
                }
    ...
    /**
     * 抽取公共的 授权与初始化逻辑
     * needAutoSaveLicense 核心解释：
     * 设为 true 时，授权成功后内核会自动将 License 缓存到本地。
     * 这样二次启动时，即使设备处于无网或弱网状态，内核也能读取本地缓存直接完成鉴权，保证业务的稳定可用。
     * 若设为 false，每次进程重启均需联网校验，网络异常会导致 X5 降级为系统内核。
     *
     */
    private fun doAuthAndInit(context: Context, needAutoSaveLicense: Boolean) {
        TbsFramework.authenticateX5(needAutoSaveLicense, object : IAuthRequestCallback {
